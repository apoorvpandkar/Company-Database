...
def readLock(operation):
    ...
    if dataItem not in lockTable_objects: #if the item is not in locked list
        lockTable_objects[dataItem] = {"state" : "read", "lockedBy": [transactionID]}
        ...
        if lockTable_objects[dataItem]["state"] == "unlocked": #if the lock state is unlock, read
            ...
            lockTable_objects[dataItem]["lockedBy"].append(transactionID)
            ...
            if lockTable_objects[dataItem]["state"] == "read": # if there is a shared lock applied
                if transactionID not in lockTable_objects[dataItem]["lockedBy"]:
                    lockTable_objects[dataItem]["lockedBy"].append(transactionID)
                if dataItem not in transaction_objects[transactionID]["lockItems"]:
                    transaction_objects[transactionID]["lockItems"].append(dataItem)
                ...
                holdingDataItems = lockTable_objects[dataItem]["lockedBy"]
...
def writeLock(operation):
    ...
    if dataItem not in lockTable_objects: #if data item is not in lock table
        lockTable_objects[dataItem] = {"state" : "write", "lockedBy": [transactionID]}
        transaction_objects[transactionID]["lockItems"].append(dataItem)
        ...
        if lockTable_objects[dataItem]["state"] == "unlocked": #if the state is unlocked, changed the state to write
            ...
            holdingTidList = lockTable_objects[dataItem]["lockedBy"]
...
def woundWait(reqTid, holdingTid, dataItem, opr, operation):
    ...
    if transaction_objects[reqTid]["timestamp"] < transaction_objects[holdingTid]["timestamp"]: #checks the timestamp to satisfy wound wait condition
        ...
        if reqTid not in lockTable_objects[dataItem]["lockedBy"]:
            lockTable_objects[dataItem]["lockedBy"].append(reqTid)
    ...
    for id in holding_ids:
        if curID is not id:
            if transaction_objects[id]["timestamp"] < temp:
                ...
                oldTid = id #using for loop check time stamp of all items in holding ids to check the oldest transaction there
    return oldTid
...
def abortOrCommit(transactionID, isCommitted):
    ...
    #Remove transaction Id from list of lockedBy
    for i in range(0, len(transaction_objects[transactionID]["lockItems"])):
        item = transaction_objects[transactionID]["lockItems"][i]
        if transactionID in lockTable_objects[item]["lockedBy"]:
            lockTable_objects[item]["lockedBy"].remove(transactionID)
        #If item is not being accessed by any transaction change state  of item to unlocked
        if len(lockTable_objects[item]["lockedBy"]) == 0:
            lockTable_objects[item]["state"] = "unlocked"

    #Remove the item from list of transaction ID's lockItems 
...
def startWaitingTransaction(transaction_objects, lockTable_objects):
    ...
    if len(waiting_transactions) == 1:
        operator(waiting_transactions.pop(0), False) #isCommitted set to False

    else:
        operator(waiting_transactions.pop(0), True) #isCommitted set to True
...
def operator(operation, flag):
    ...
    if flag and len(waiting_transactions) > 0: #If flag is true transaction added to waiting transaction is executed
        startWaitingTransaction(transaction_objects, lockTable_objects)


#Read input file
file = open("input.txt", "r") #Input file
lines = file.readlines()

#Please change the name of the output file for different input cases here
log_file = open("output.txt", 'w+') #Output written to file output.txt

for line in lines:
    operation = line.strip()
    operation = operation.replace(" ", "") #Remove spaces between operation
